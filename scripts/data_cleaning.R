...
Episode_number <- substring(filenames, 20)
...
write.csv(raw,"data/raw_data/raw.csv", row.names = FALSE)  
...
# read in raw data
raw <- read.csv(file = 'data/raw_data/raw.csv')
...
clean <- clean %>% separate(textString, c("Speaker1","Text1"), sep = ":", 
                            fill = "left", remove=FALSE, extra = "merge")
clean <- clean %>% separate(Text1, c("Speaker2","Text2"), sep = ":", 
                            fill = "left", remove=FALSE, extra = "merge")
clean <- clean %>% separate(Text2, c("Speaker3","Text3"), sep = ":", 
                            fill = "left", remove=FALSE, extra = "merge")
clean <- clean %>% separate(Text3, c("Speaker4","Text4"), sep = ":", 
                            fill = "left", remove=FALSE, extra = "merge")

# combine all distinct speakers for speaker "slot"     
unique <- clean %>% distinct(select(clean, Speaker1)) %>%
  rename(Speaker=Speaker1) 

unique <- clean %>% distinct(select(clean, Speaker2)) %>%
  rename(Speaker=Speaker2) %>% 
  bind_rows(unique,.)

unique <- clean %>% distinct(select(clean, Speaker3)) %>%
  rename(Speaker=Speaker3) %>% 
  bind_rows(unique,.)                                         

unique <- clean %>% distinct(select(clean, Speaker4)) %>%
...
brackets <- clean %>% 
  ...
  mutate(total_words <- sapply(Speaker1, 
                               function(x) length(unlist(
                                 strsplit(as.character(x), "\\W+"))))) %>% 
...
comma_and	<- comma_and %>% mutate(Speaker=replace(Speaker,
...
testing <- testing %>% mutate(Speaker=replace(Speaker,Temp!=Speaker,NA)) %>%
  select(-c(Temp))  %>%
  drop_na(Speaker)
...
# last manual edit:
last_drops <- c(" csi", " one"," yes", "male", "na 00", " alone", " first", 
...
for (entry in list_Speaker){
  clean <- clean %>% mutate(actual_Speaker_1 = replace(actual_Speaker_1,
                                                       grepl(entry,Speaker1, ignore.case = TRUE),
                                                       entry),
                            actual_Speaker_2 = replace(actual_Speaker_2,
...
clean <- clean %>%
  rowwise() %>% 
  mutate(actual_Speaker_1 = replace(actual_Speaker_1,
...
clean$actual_text_1 <- ifelse(
  ...
  clean$text,
  str_replace(clean$text, 
              clean$actual_Speaker_1, 
              "####"))
clean$actual_text_1 <-str_replace(clean$actual_text_1, 
                                  "####", 
                                  "") 
...
clean$actual_text_1 <- ifelse(
  ...
  clean$actual_text_1,
  str_replace(clean$actual_text_1, 
              clean$actual_Speaker_2, 
              "####"))

clean <- clean %>% separate(actual_text_1, c("actual_text_1","actual_text_2"), 
                            sep = "#### ", 
                            fill = "left", 
                            remove= FALSE, 
                            extra = "merge") %>% select(-text)

# edit missmatches:
clean <- clean %>% 
  rowwise() %>% 
  mutate(actual_text_1 = replace(actual_text_1,is.na(actual_text_1),actual_text_2)) %>%  
  mutate(actual_text_2 = replace(actual_text_2, actual_text_1==actual_text_2, NA))
# clean up
clean$actual_text_1 <-str_replace(clean$actual_text_1, 
                                  "####", 
                                  "")   
...
clean$actual_text_2 <- ifelse(
  ...
  clean$actual_text_2,
  str_replace(clean$actual_text_2, 
...
clean <- clean %>% 
  rowwise() %>% 
  mutate(actual_text_2 = replace(actual_text_2,is.na(actual_text_2),actual_text_3)) %>%  
  mutate(actual_text_3 = replace(actual_text_3, actual_text_3==actual_text_2, NA))

# clean up
clean$actual_text_2 <-str_replace(clean$actual_text_2, 
                                  "####", 
                                  "")      
...
# Actor 1
Actor1 = select(clean, -c(actual_Speaker_2,actual_Speaker_3,
                          actual_text_2,actual_text_3))
Actor1 <- Actor1 %>%
  rename(Actor=actual_Speaker_1,
         Text=actual_text_1)
Actor1$number <- 1
# Actor 2      
Actor2 =select(clean,-c(actual_Speaker_1,actual_text_1,
                        actual_Speaker_3,actual_text_3))
Actor2 <- Actor2 %>% drop_na(actual_Speaker_2)
Actor2 <- Actor2 %>%
  rename(Actor=actual_Speaker_2,
         Text=actual_text_2)
Actor2$number <- 2
# Actor3        
Actor3 =select(clean,-c(actual_Speaker_1,actual_text_1,
                        actual_Speaker_2,actual_text_2))
Actor3 <- Actor3 %>% drop_na(actual_Speaker_3)
Actor3 <- Actor3 %>%
  rename(Actor=actual_Speaker_3,
         Text=actual_text_3)
Actor3$number <- 3
# combine      
...
clean <- clean %>% 
  ungroup() %>% 
  select(c(Episode_number, startTime, endTime, Actor, Text, textString))
...
Marisha3 <- data.frame("Actor"=c("marsha","mairsha","marishaa","marishah",
...
Off <- as.list(c("offscreen","off-screen voice #1","off-screen voice #2","off member",
...
clean$Actor <- ifelse(clean$Actor=="chris" & clean$Episode_number=="046","chrisH",clean$Actor)
clean$Actor <- ifelse(clean$Actor=="chris" & clean$Episode_number=="055","chrisP",clean$Actor)
...
clean <- clean %>% mutate(Actor=replace(Actor,
                                        Actor=="pretending trinket and travis", 
                                        "travis")) %>% 
  mutate(Actor=replace(Actor,
...
# dropping all which is contained within brackets, 
# which indicate descriptive text, like Matt: "(heavy accent) Hi"
...
# Episode numbers manual adjustments
# kick out -1 ending in an episode number as it indicates that the episodes
# is split into two videos. The file with -1 indicates it's the first video.
clean$Episode_number <-  gsub("-1", "", clean$Episode_number)
# there are 3 episodes which are split in two: 31,33,35
second_episodes <- as.list(c("031-2","033-2","035-2"))
...
clean <- clean %>% 
  mutate(Episode_number = replace(Episode_number, Episode_number == "115-GROG","116"),
         ...
         Episode_number = replace(Episode_number, Episode_number == "115-VAMP1","118"),
         Episode_number = replace(Episode_number, Episode_number == "115-VAMP2","119"))

# add arcs
clean <- clean %>%
  mutate(Episode_number = as.numeric(Episode_number)) %>% 
  mutate(Arc = NA) %>% 
  mutate(Arc=replace(Arc,Episode_number <=16 & is.na(Arc),"Kraghammer"),
         Arc=replace(Arc,Episode_number <=23 & is.na(Arc),"Vasselheim"),
         Arc=replace(Arc,Episode_number <=38 & is.na(Arc),"Briarwoods"),
         Arc=replace(Arc,Episode_number <=56 & is.na(Arc),"Attack_Conclave"),
         Arc=replace(Arc,Episode_number <=69 & is.na(Arc),"Vestiges"),
         Arc=replace(Arc,Episode_number <=84 & is.na(Arc),"Fall_Conclave"),
...
# Break stamps
time_stamps <- read.csv("./data/time_stamps/Running Times - Tal'Dorei - TD Campaign.csv",
                        na.strings=c("","NA"))
...
# Combat stamps
combat_stamps <- read.csv("./data/time_stamps/Running Times - Tal'Dorei - TD Combat Times.csv",
                          na.strings=c("","NA"))
...
time_stamps <- left_join(time_stamps,combat_stamps, by = "Episode") %>% 
  ...
  mutate(Episode_number=gsub("C1E", "", Episode_number)) %>% 
  mutate(Episode_number=as.numeric(Episode_number)) 
...
time_stamps <- time_stamps %>% rowwise() %>% 
  mutate(second_end = replace(second_end,
                              is.na(second_end),
                              first_end)) %>% 
  mutate(first_end=replace(first_end,
...
clean <- clean %>% 
  ...
  mutate(segment = replace(segment, startTime <= first_start, "proglog")) %>% 
...
combat_stamps <- time_stamps %>% 
  ...
  pivot_wider(names_from = encounter_number, 
